...
$ git rm -r catalog1        # удалить каталог из индекса
...
$ git log -p -2             # просмотреть журнал коммитов и изменения в файлах
$ git diff HEAD~1 HEAD      # просмотреть изменения между последним коммитом и его родителем
...
```
# Позволяет сначала "перемотать" твои изменения на временную ветку, подтянуть последние изменения из master, а затем применить твои коммиты поверх них
$ git pull --rebase
# Разреши конфликты вручную (отредактируй конфликтные файлы)
# затем
$ git add .
# Закончить процесс слияния (merge) или ребейса (rebase)
$ git rebase --continue
